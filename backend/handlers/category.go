...
func GetCategories(c *gin.Context) {
	rows, err := db.Pool.Query(c, "SELECT id, name, icon, color, parent_id, type FROM categories ORDER BY type, name")
	if err != nil {
...
func CreateCategory(c *gin.Context) {
	...
	err := db.Pool.QueryRow(c,
		`INSERT INTO categories (name, icon, color, parent_id, type) VALUES ($1, $2, $3, $4, $5)
		 RETURNING id, name, icon, color, parent_id, type`,
		cat.Name, cat.Icon, cat.Color, cat.ParentID, cat.Type,
