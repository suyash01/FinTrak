  ...
  return (
    <>
      <div className="shrink-0 px-8 pt-6">
        ...
        <p className="text-slate-400 text-sm">Manage your bank accounts and credit cards</p>
      </div>
      <div className="flex-1 px-8 pb-8 pt-6 overflow-y-auto w-full">
        <div className="flex justify-end mb-5">
          <button className="inline-flex items-center gap-2 px-4 py-2 bg-linear-to-r from-cyan-500 to-blue-600 text-white rounded-lg text-sm font-medium hover:opacity-90 transition-all shadow-lg shadow-cyan-500/20" onClick={() => setShowNew(true)}>
        ...
        {showNew && (
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 mb-5">
            <div className="flex justify-between items-center mb-4">
              ...
              <button className="p-1.5 text-slate-400 hover:text-slate-200 hover:bg-slate-800 rounded-lg transition-colors" onClick={() => setShowNew(false)}><X size={16} /></button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-5">
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-medium text-slate-400">Name</label>
                <input className="px-3.5 py-2.5 bg-slate-950 border border-slate-800 rounded-lg text-slate-200 text-sm focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-all" placeholder="e.g. HDFC Savings" value={newAcc.name} onChange={(e) => setNewAcc({ ...newAcc, name: e.target.value })} />
              </div>
              <div className="flex flex-col gap-1.5">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-medium text-slate-400">Bank</label>
                <input className="px-3.5 py-2.5 bg-slate-950 border border-slate-800 rounded-lg text-slate-200 text-sm focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-all" placeholder="e.g. HDFC" value={newAcc.bank} onChange={(e) => setNewAcc({ ...newAcc, bank: e.target.value })} />
              </div>
              <div className="flex flex-col gap-1.5">
        ...
        {accounts.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 px-4 text-center">
            ...
            <p className="text-slate-400 text-sm mb-6 max-w-md">Add a bank account or credit card to start importing statements and categorizing your transactions.</p>
            <button className="inline-flex items-center gap-2 px-4 py-2 bg-linear-to-r from-cyan-500 to-blue-600 text-white rounded-lg text-sm font-medium hover:opacity-90 transition-all shadow-lg shadow-cyan-500/20" onClick={() => setShowNew(true)}>Add Account</button>
          </div>
        ) : (
          <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 ${compactLayout ? 'gap-3' : 'gap-5'}`}>
            {accounts.map((acc) => (
              <div key={acc.id} className={`bg-slate-900 border border-slate-800 rounded-xl ${compactLayout ? 'p-3' : 'p-5'} hover:border-slate-700 transition-colors flex flex-col`} style={{ borderLeftColor: editingId === acc.id ? editAcc.color : acc.color, borderLeftWidth: '3px' }}>
                {editingId === acc.id ? (
                  <div className="flex flex-col h-full">
                    <div className="flex justify-between items-center mb-4">
                      <h4 className="text-sm font-semibold">Edit Account</h4>
                      <button className="p-1.5 text-slate-400 hover:text-slate-200 hover:bg-slate-800 rounded-lg transition-colors" onClick={() => setEditingId(null)}><X size={14} /></button>
                    </div>
                  ...
                  <div className="flex justify-between items-start h-full">
                    ...
                    <div className="flex gap-1 -mr-2 -mt-2">
                      <button className="p-2 text-slate-400 hover:text-cyan-400 hover:bg-slate-800 rounded-lg transition-colors" onClick={() => handleExport(acc.id)} title="Export CSV">
                        <Download size={16} />
                      </button>
                      <button className="p-2 text-slate-400 hover:text-white hover:bg-slate-800 rounded-lg transition-colors" onClick={() => startEdit(acc)} title="Edit Account">
                        <Edit2 size={16} />
                      </button>
                      <button className="p-2 text-slate-400 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors" onClick={() => handleDelete(acc.id)} title="Delete Account">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  </div>
