  ...
  return (
    <>
      ...
      <div className="flex-1 px-8 pb-8 pt-6 overflow-y-auto w-full">
        {/* Stats */}
        <div className={`grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 ${compactLayout ? 'gap-3 mb-4' : 'gap-5 mb-6'}`}>
          <div className={`bg-slate-900 border border-slate-800 rounded-xl ${compactLayout ? 'p-3' : 'p-5'} hover:border-slate-700 transition-colors`}>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 hover:border-slate-700 transition-colors">
            ...
            <div className="text-2xl font-bold text-red-500">{formatCurrency(data.totalExpense)}</div>
          </div>
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 hover:border-slate-700 transition-colors">
            ...
            <div className={`text-2xl font-bold ${netSavings >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
              {formatCurrency(netSavings)}
            </div>
          </div>
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 hover:border-slate-700 transition-colors">
            <div className={`w-11 h-11 rounded-lg flex items-center justify-center bg-violet-500/15 ${compactLayout ? 'mb-2' : 'mb-3'}`}>
              <ArrowUpDown size={22} className="text-violet-500" />
            </div>
            <div className="text-xs text-slate-400 mb-1">Total Transactions</div>
            <div className={`${compactLayout ? 'text-xl' : 'text-2xl'} font-bold text-slate-100`}>{data.totalTransactions.toLocaleString()}</div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 mb-6">
          {/* Monthly Trend */}
          <div className={`bg-slate-900 border border-slate-800 rounded-xl ${compactLayout ? 'p-4' : 'p-6'} flex flex-col w-full`}>
            <div className={`flex items-center justify-between ${compactLayout ? 'mb-3' : 'mb-5'}`}>
              <h3 className="text-base font-semibold">Monthly Income vs Expenses</h3>
            </div>
        ...
        {/* Recent Transactions */}
        <div className={`bg-slate-900 border border-slate-800 rounded-xl ${compactLayout ? 'p-4' : 'p-6'}`}>
          <div className={`flex items-center justify-between ${compactLayout ? 'mb-3' : 'mb-5'}`}>
          ...
          {data.recentTransactions.length > 0 ? (
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr>
                    <th className={`${compactLayout ? 'py-1.5 px-3' : 'py-3 px-4'} text-xs font-semibold uppercase tracking-wider text-slate-500 bg-slate-800/50 border-b border-slate-800 whitespace-nowrap`}>Date</th>
                    <th className={`${compactLayout ? 'py-1.5 px-3' : 'py-3 px-4'} text-xs font-semibold uppercase tracking-wider text-slate-500 bg-slate-800/50 border-b border-slate-800 whitespace-nowrap`}>Description</th>
                    ...
                    <th className={`${compactLayout ? 'py-1.5 px-3' : 'py-3 px-4'} text-xs font-semibold uppercase tracking-wider text-slate-500 bg-slate-800/50 border-b border-slate-800 whitespace-nowrap text-right`}>Amount</th>
                  </tr>
                </thead>
                <tbody>
                  {data.recentTransactions.map((t) => (
                    <tr key={t.id} className="hover:bg-slate-800/30 transition-colors">
                      <td className={`${compactLayout ? 'py-1.5 px-3' : 'py-3 px-4'} text-sm border-b border-slate-800`}>{formatDate(t.date)}</td>
                      <td className={`${compactLayout ? 'py-1.5 px-3' : 'py-3 px-4'} text-sm border-b border-slate-800 max-w-[300px]`}>
                        <div className="font-medium text-slate-200 truncate">{t.description}</div>
                        {t.payee && <div className="text-[11px] text-slate-500 truncate">{t.payee}</div>}
                      </td>
                      <td className={`${compactLayout ? 'py-1.5 px-3' : 'py-3 px-4'} text-sm border-b border-slate-800`}>{t.accountName}</td>
                      <td className={`${compactLayout ? 'py-1.5 px-3' : 'py-3 px-4'} text-sm border-b border-slate-800`}>
                        {t.categoryName ? (
                          <span className={`${compactLayout ? 'px-2 py-0' : 'px-2.5 py-0.5'} inline-flex items-center rounded-full text-xs font-medium border`}
                                style={{ color: t.categoryColor, borderColor: t.categoryColor, background: `${t.categoryColor}15` }}>
                            {t.categoryName}
                          </span>
                        ) : (
                      ...
                      <td className={`${compactLayout ? 'py-1.5 px-3' : 'py-3 px-4'} text-sm border-b border-slate-800 text-right font-semibold`}>
...
function CategoryPieSection({ title, categories, emptyMessage }) {
  const { compactLayout } = useSettings();
  return (
    <div className={`bg-slate-900 border border-slate-800 rounded-xl ${compactLayout ? 'p-4' : 'p-6'} flex flex-col`}>
      <div className={`flex items-center justify-between ${compactLayout ? 'mb-3' : 'mb-5'}`}>
            ...
            <div className="w-full md:w-1/2 text-[13px] max-h-[280px] overflow-y-auto pr-2">
              {categories.map((cat, i) => (
                <div key={i} className="flex items-center gap-2 py-1.5 border-b border-slate-800 last:border-0">
                  <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ background: cat.categoryColor }} />
                  <span className="flex-1 truncate">{cat.categoryName}</span>
                  <span className="font-medium text-slate-200">{formatCurrency(cat.total)}</span>
                </div>
              ))}
            </div>
          </div>
