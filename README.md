...
- The production nginx config serves `/sw.js` and `/manifest.webmanifest` with `no-cache` (the hashed-asset rule would otherwise pin a worker for a year) and needs no CSP change: `manifest-src`/`worker-src` fall back to `default-src 'self'` and `script-src 'self'`.
