...
- `TRUSTED_PROXIES` (comma-separated proxy CIDRs) decides whether `X-Forwarded-For` / `X-Real-IP` are believed, and therefore whether per-IP rate limiting sees real client addresses. Both production Compose files set it for you, defaulting to the pinned `app-net` subnet (`172.28.0.0/24`) that the frontend nginx proxies from — every browser request reaches the API through that proxy, so without it all users would share one auth rate-limit bucket. Override `APP_SUBNET` to move the network and the default together if that subnet collides with an existing Docker network on the host. Leave it empty only when the backend is exposed directly: with no trusted proxies configured, forwarded headers are ignored and cannot be spoofed.
...
- **Shell**: the worker precaches the shell files and the bundles named by `index.html`, then the app reports the assets its page loaded (`src/lib/prefetchRoutes.ts`) and warms the two routes that must work offline — dashboard and transactions. Other pages need one online visit before they open offline.
...
- The production nginx config serves `/sw.js` and `/manifest.webmanifest` with `no-cache` (the hashed-asset rule would otherwise pin a worker for a year) and repeats the server-level security-header block in those locations, because a location that defines its own `add_header` does not inherit them. The CSP itself needs no change: `manifest-src`/`worker-src` fall back to `default-src 'self'` and `script-src 'self'`.
...
Five `GET`s in this API are not pure reads: the billing-cycle handlers materialize a credit-card account's statement periods on read (and back-fill transactions' cycle assignment), so `list_billing_cycles`, `list_transactions` (with an `accountId`), `get_dashboard_summary`, `get_money_flow_timeline` and `get_cash_flow_calendar` can write derived cycle rows. They are listed in `readonly.SideEffectingGETs`, each of those tools carries a `SideEffect` that is appended to the description the model reads, and the audit fails if either side of that pair changes without the other. A model's suggestions therefore have to be confirmed in the app — which is exactly the app's own ethos: it never auto-categorizes, auto-links or auto-creates either.
